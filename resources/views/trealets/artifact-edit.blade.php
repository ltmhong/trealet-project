...
@section('content')
<div id = "myLoader"></div>
<div class="artifact-composer-container">
    <div class="artifact-composer__header">
        <div class="header__list">
            ...
            <ul class="tab" id="step-number">
                ...
                @foreach($artifact['items'] as $key =>$val)
    ...
    <div class="artifact-composer__content" id="steps">
            ...
            <form id="main-form">
                <div class="d-flex mb-2 py-1">
                    ...
                    <div class="flex-grow-1">
                        <input type="text" class="w-100 form-control" id = "name" name="title" placeholder="Nhập tiêu đề" value="{{ $artifact['title']?? '' }}">
                    </div>
                </div>
                <div class="d-flex mb-2 py-1">
                    <div class="text-left w-110 font-500">Mô tả<span style="color:red"> *</span></div>
                    <div class="flex-grow-1">
                        <textarea class="w-100 no-resize form-control" name="des" id= "des" rows="3"  placeholder="Nhập địa điểm tham quan nếu có">{{ $artifact['des'] ?? '' }}</textarea>
                    </div>
                </div>
                <div class="d-flex mb-2 py-1">
                    <div class="text-left w-110 font-500">Điểm số tối thiểu để nhận thưởng</div>
                    <div class="flex-grow-1">
                        @if ($artifact['minScore'] == "1")
                ...
                <div class="d-flex mb-2 py-1">
                    ...
                    <div class="flex-grow-1">
                        <input type="text" class="w-100 form-control" name="gift" placeholder="Nhập phần thưởng" value="{{ $artifact['gift']?? '' }}">
                    </div>
                </div>
                <div class="d-flex mb-2 py-1">
                    <div class="text-left w-110 font-500">Ngôn ngữ</div>
                    <div class="flex-grow-1">
                        @if ($artifact['language'] == "vn")
                            <select class="w-100 no-resize form-control" value="{{ $artifact['language']?? '' }}" name = "language" id = "language" aria-label="Default select example" rows="4">
                                    ...
                                    <div class="">
                                        ...
                                        <div class="w-100">
                                            <div class="custom-control custom-radio">
                                                ...
                                                <div class="d-flex align-items-center">
                                        <label style="min-width: 60px;text-align:center" for="">Giải thích</label>
                                        <textarea type="text" class="hint form-control mb-2 ml-2">@if($val['type']=='Jigsaw' ) {{ $val['hint'] ??'' }}@endif</textarea>
                                    </div>
                                            </div>
                                        </div>
...
<script>
    ...
    (function() {
        $.ajax({
            url: "{{ route('artifact-edit.tree-folder') }}",
            success: function(res) {
                let data = JSON.parse(res);
                let max = "{{ count($artifact['items'] ) }}"
        ...
        $(document).on('change', `#step${currentSteps + 1} .upload-field-input`, async function() {
            ...
            if(document.getElementById("selectType" + String(currentSteps + 1)).value == "quizz"){
                ...
                input.setAttribute("type-data" , "");
                    ...
                    input = document.getElementById(`step${currentSteps + 1}`).querySelectorAll(".upload-field-input")[2];
                input.setAttribute("type-data" , "");
            }
            else{
    ...
    async function changeFileInput(e, files) {
            ...
            else if(document.getElementById(`selectType` + String(indexStep)).value == "jigsaw"){
                    container = document.getElementById(`step` + String(indexStep)).querySelectorAll(".upload-field-preview")[2];
            }
            else{
    ...
    $(document).on('click', '#save', function(e) {
        ...
        document.getElementsByClassName("artifact-composer-container")[0].style.opacity = 0.1;
        document.getElementById("myLoader").style.display = "block";
        if(document.getElementById("name").value == ""){
            alert("Bạn chưa nhập tên hiện vật!!")
            document.getElementsByClassName("artifact-composer-container")[0].style.opacity = 1;
            document.getElementById("myLoader").style.display = "none";
            return;
        }
        if(document.getElementById("des").value == ""){
            alert("Bạn chưa nhập mô tả của hiện vật!!")
            document.getElementsByClassName("artifact-composer-container")[0].style.opacity = 1;
            document.getElementById("myLoader").style.display = "none";
            return;
        }
        ...
        if (validData) {
            $.ajax({
                url: "{{ route('artifact-edit.update', $id) }}",
                type: 'post',
                contentType: false,
                processData: false,
                data: formData,
...
@keyframes spin {
    ...
    .artifact-composer__content .step {
        height: 100%;
        overflow: hidden;
    }

    .artifact-composer__content .step#info {
        height: fit-content;
    }
    ...
    .artifact-composer__content .form-wrap {
        overflow-x: hidden;
        min-width: 550px;
    }
