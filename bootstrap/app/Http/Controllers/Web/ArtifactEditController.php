...
class ArtifactEditController extends Controller
{
    public function index()
    {
        $albums = $this->getAlbum();

        return view('trealets.artifact', compact('albums'));
    }
    ...
    public function update($id, Request $request)
    {
        ...
        $artifact = ['trealet' => array_merge($main, ['items' => $request->get('items')])];
        ...
        DB::update(
            ...
            [
                auth()->id(),
                $main['title'],
                json_encode($artifact, JSON_UNESCAPED_UNICODE),
                $id,
            ]
        );
