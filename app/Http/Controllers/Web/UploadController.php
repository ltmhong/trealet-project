...
class UploadController extends Controller
{
    ...
    public function artifact_upload(Request $request)
    {
        if(!request()->hasFile('artifact_file')) return response()->json(['message' => 'No mode; file uploaded'],400);
        $file = request()->file('artifact_file');

        $realpath = $file->getRealPath();
    
        $name = date('YmdHis').random_int(10, 99);
        $path = $this->insert_new_artifact($realpath, $name); //date('YmdHis') );
        // return response()->json(['data' => $path], 200);
        return $path;
    }

    public function artifact_upload_api(Request $request)
    {
        if(!request()->hasFile('artifact_file')) return response()->json(['message' => 'No artifact file uploaded'],400);
        $file = request()->file('artifact_file');
    ...
    function insert_new_artifact($from, $name)
    {
        $output_glb = request()->server('DOCUMENT_ROOT').'/upload/trealet-data/'.$name.'.glb';
        move_uploaded_file($from, $output_glb);
        $w_output = '../upload/trealet-data/'.$name.'.glb';
        $namefile = $name.'.glb';
        $this->upload_to_nextcloud($output_glb, $namefile);
        return $w_output;
    }
